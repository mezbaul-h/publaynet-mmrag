...
\setlength{\parskip}{0pt}        % consistent: paragraphs marked by indent only
\emergencystretch=3em            % let long lines break instead of overrunning the margin

\begin{document}

\begin{center}
  {\large\bfseries Multimodal RAG + Knowledge Graph over PubLayNet}\\[3pt]
  {\small Mezbaul Haque \quad\textbullet\quad June 2026}
\end{center}
\vspace{2pt}
...
before the next, so models that cannot co-reside on a 12\,GiB card load one at a time
and re-runs skip finished work---crash-safe on modest hardware. Each backbone is
among the strongest open options that fit the budget, so the comparison reflects
retrieval \emph{design}, not under-powered parts.
\begin{itemize}
  \item \textbf{Stage 1 (OCR \& chunking).} Crops every labelled region; OCRs text
  with Surya (robust on dense multi-column layouts); packs reading-order text into
  $\le$1.2k-char \textbf{chunks}---specific yet coherent---with provenance back to
  regions.
  \item \textbf{Stage 1b (captioning).} Captions a $\sim$500-crop sample of
  figures/tables with a VLM (Qwen2.5-VL-3B), making visual content text-searchable.
  \item \textbf{Stage 2 (embedding).} Embeds chunks with BGE-M3 (dense \emph{and}
  learned-sparse from one model) and crops with SigLIP2 (shared image--text space, so
  text reaches figures) into \textbf{embedded Qdrant}---chosen over dense-only
  libraries because it stores dense+sparse per point and fuses them (RRF) in one
  query, and the same API scales to a server.
  \item \textbf{Stage 3 (knowledge graph).} Entities via GLiNER (zero-shot), relations
  as LLM triples (typed, schema-free), into a NetworkX graph (67{,}796 nodes;
  13{,}336 relations).
\end{itemize}
The result is \textbf{three aligned views} of each page---dense+sparse text, SigLIP
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.76\linewidth]{architecture-enhanced.png}
  \caption{Enhanced pipeline: hybrid-text, SigLIP-image, and KG-expansion channels
  fused by rank-weighted RRF, reranked, then answered. Baseline diagram and a worked
  HER2 example: README, \emph{How it works}.}
\end{figure}
...
\textbf{Insights.} \textbf{(1)}~A text-gold-only benchmark structurally hides multimodal/KG
value---segmented evaluation is essential. \textbf{(2)}~Graph and sparse retrieval are partly
redundant on multi-hop, but the graph captures most (0.61 vs 0.41) by reaching chunks
sharing \emph{no} surface terms with the query. \textbf{(3)}~Equal-weight fusion trades a
...
bigger VLM; for charts, where OCR does not apply, a sharper/stronger VLM is the lever (the table is shown in the README).

\section{Reproduction}

\texttt{pip install -e .}, then run stages \texttt{01}, \texttt{01b}, \texttt{02},
\texttt{03} in order, and
\texttt{04\_run\_eval.py --variants baseline,\allowbreak abl\_rerank,\allowbreak abl\_hybrid,\allowbreak abl\_image,\allowbreak abl\_graph,\allowbreak enhanced}
